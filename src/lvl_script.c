        ...
        case Cmd_DRAWFROM:{
            ...
            if (level_file_version > 0)
            {
                ...
                for (fi = 0, ri = 0; fi < COMMANDDESC_ARGS_COUNT; fi++, ri++)
                {
                    ...
                    if ((chr == 'A' && !(parameter_is_digit(funscline->tp[fi+1]))) && (!is_if_statement)) // Strings don't have a range, but IF statements have 'Aa' to allow both variable compare and numbers. Numbers are allowed, 'a' is a string for sure.
                    {
