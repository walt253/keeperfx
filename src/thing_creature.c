...
void process_creature_pooping_gold(struct Thing *thing)
{
    ...
    struct CreatureStats* crstat = creature_stats_get_from_thing(thing);
    if ((crstat->gold_pooping_amount > 0) && (crstat->gold_pooping_frequency > 0))
    {
        if (((game.play_gameturn + thing->index) % crstat->gold_pooping_frequency) == 0)
        {
            drop_gold_pile(crstat->gold_pooping_amount, &thing->mappos);
        }
    }
    return;
}
